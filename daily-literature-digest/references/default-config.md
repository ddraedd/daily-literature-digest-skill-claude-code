...
The `language` field controls the digest's writing language (e.g. `en`, `zh-CN`). The `crossref_mailto` is sent in the API User-Agent so Crossref/OpenAlex route you to their polite pool — use the user's email.
...
To customize publishers, replace the omitted key with a list of objects shaped like:
...
- Create a separate follow-up list for title-only candidates. The user logs in themselves; Claude can then process that explicit batch using the active session or local PDFs.
