#!/usr/bin/env python3
"""Fetch daily literature candidates for a Claude Code-authored digest.

This script intentionally does not call an LLM. It gathers open metadata and
abstracts, then writes a JSON payload for a Claude Code automation to summarize.
"""
...
USER_AGENT_BASE = "ClaudeDailyLiteratureDigest/1.0"
...
def apply_runtime_config(args: argparse.Namespace) -> None:
    ...
    if args.command == "fetch":
        ...
        output_dir = clean_text(config.get("output_dir")) or str(DEFAULT_OUTPUT_DIR)
        args.state_file = args.state_file or clean_text(config.get("state_file")) or str(Path(output_dir) / "state.json")
